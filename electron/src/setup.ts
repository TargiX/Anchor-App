...
function isAllowedAppUrl(url: string, customScheme: string): boolean {
  try {
    return new URL(url).protocol === `${customScheme}:`
  } catch {
    return false
  }
}
...
export class ElectronCapacitorApp {
  ...
  async init(): Promise<void> {
    ...
    this.MainWindow.webContents.setWindowOpenHandler((details) => {
      return isAllowedAppUrl(details.url, this.customScheme)
        ? { action: "allow" }
        : { action: "deny" }
    })
    this.MainWindow.webContents.on("will-navigate", (event, newURL) => {
      if (!isAllowedAppUrl(newURL, this.customScheme)) {
...
export function setupContentSecurityPolicy(customScheme: string): void {
  const appSource = `${customScheme}://*`
  const scriptSource = electronIsDev
    ? `${appSource} 'unsafe-inline' 'unsafe-eval' devtools://*`
    : `${appSource} 'unsafe-inline'`
  const policy = [
    `default-src ${appSource}`,
    `script-src ${scriptSource}`,
    `style-src ${appSource} 'unsafe-inline'`,
    `img-src ${appSource} data: blob:`,
    `font-src ${appSource} data:`,
    `connect-src ${appSource} https://*.supabase.co wss://*.supabase.co`,
    `object-src 'none'`,
    `base-uri 'none'`,

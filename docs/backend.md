...
| `PORT` | `3000` |
| `RESEND_API_KEY` | Resend sending key (local copy: `~/.ssh/hetzner-apps/uptime-resend.json`) — without it reset emails are logged, not sent |
| `AUTH_EMAIL_FROM` | `Anchor <no-reply@phosphene.cc>` — `phosphene.cc` is already a verified Resend domain |

...
export function AnchorVoiceCheckInMvp() {
  ...
  useEffect(() => {
    ...
    async function loadCheckIns() {
      try {
        ...
        captureClientException(err, { flow: "voice_check_in_load" })
        if (!cancelled) {
          setError(
            err instanceof Error
              ? err.message
              : "Не смог загрузить локальные чек-ины."
          )
        }
      }
    }
  ...
  const latestMain = useMemo(
    () =>
      [...records].reverse().find((record) => record.intention.main)?.intention
        .main,
    [records]
  )
  ...
  async function submitCheckIn() {
    ...
    try {
      ...
      if (data.checkIn)
        setRecords((prev) => [...prev, data.checkIn as AnchorCheckIn])
      setReflection(data.reflection ?? "")
      setSubmittedKind(data.checkIn?.kind ?? kind)
      setResetStep(
        data.checkIn?.kind === "spontaneous"
          ? (data.checkIn.mentions.find((mention) => mention.kind === "task")
              ?.text ?? null)
          : null
      )
      setDigest(data.digest ?? "")
      ...
      captureClientException(err, { flow: "voice_check_in_submit", kind })
  ...
  function startBrowserVoice() {
    ...
    if (!SpeechRecognition) {
      captureEvent("voice_input_unavailable")
      setError(
        "В этом браузере нет Web Speech API. Пока просто вставь текст расшифровки."
      )
      return
    }
    ...
    recognition.onerror = () => {
      captureEvent("voice_input_failed")
    ...
    setListening(true)
    captureEvent("voice_input_started", { language: recognition.lang })
    recognition.start()
  }
        ...
        <section className="rounded-[2rem] border border-border bg-card/70 p-5 shadow-sm lg:p-8">
          <div className="mb-7 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-4 max-w-2xl text-sm leading-6 text-muted-foreground lg:text-base">
                Наговори кашу на 10–30 секунд. MVP вытащит состояние, сон, одну
                главную вещь и хвосты. Это проверка петли, не красивая аппка.
              </p>
            </div>
          ...
          <div className="mb-4 grid grid-cols-3 gap-2 rounded-2xl bg-muted/45 p-1">
            {(["morning", "evening", "spontaneous"] as const).map((option) => (
              <button
                ...
                className={`rounded-xl px-3 py-2 text-sm transition outline-none focus-visible:ring-2 focus-visible:ring-ring ${
                  kind === option
          ...
          {kind === "spontaneous" ? (
            ...
            >
              ...
              <p className="mt-1 text-sm leading-6 text-muted-foreground">
                Без отчёта. Назови, что сейчас громче всего, и один следующий
                шаг, к которому можно вернуться.
              </p>
            </div>
          ) : null}
          ...
          {kind === "evening" && latestMain ? (
            <div className="mb-4 rounded-2xl border border-primary/20 bg-primary/5 p-4 text-sm text-muted-foreground">
              Утренний якорь:{" "}
              <span className="font-medium text-foreground">{latestMain}</span>
            </div>
          ) : null}
          ...
          <textarea
            ...
            aria-label={
              kind === "spontaneous"
                ? "Что сейчас громче всего и один следующий шаг"
                : "Текст чек-ина"
            }
            className="min-h-44 w-full resize-none rounded-2xl border border-border bg-background/70 p-4 text-base leading-7 text-foreground transition outline-none focus:border-primary/60 focus:ring-4 focus:ring-primary/10"
          />
        ...
        <aside className="space-y-4">
          ...
          <section className="rounded-[1.75rem] border border-border bg-card/70 p-5 shadow-sm">
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              Хранится локально на машине в JSONL. Без стриков, без оценок
              продуктивности, без терапевтического театра.
            </p>
          </section>
        </aside>
      </div>
